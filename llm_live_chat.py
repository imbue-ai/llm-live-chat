...
def register_commands(cli):
    ...
    def live_chat(
        ...
        if conversation_id or _continue:
            try:
                conversation = load_conversation(
                    conversation_id, database=database
                )
        ...
        try:
            ...
            raise click.ClickException(
                "'{}' is not a known model".format(model_id)
            )
        ...
        try:
            ...
            argument_fragments = [
                f for f in fragments_and_attachments if isinstance(f, Fragment)
            ]
            argument_attachments = [
                a
                for a in fragments_and_attachments
        ...
        click.echo(
            "Type '!fragment <my_fragment> [<another_fragment> ...]'"
            ...
            def _display_new_responses():
                ...
                try:
                    ...
                    if seen:
                        ...
                        where = (
                            "conversation_id = ? AND id NOT IN ({})".format(
                                placeholders
                            )
                        )
                        ...
                        params = [conv_id]
                    new_responses = list(
                        check_db["responses"].rows_where(
                            where, params, order_by="datetime_utc"
                        )
                    )
                except Exception as ex:
                    sys.stderr.write(
                        "live-chat: error checking for new responses: {}\n".format(ex)
                    )
                    return False
            ...
            while True:
                ...
                if prompt.strip() == "!edit":
                    ...
                    if edited_prompt is None:
                        click.echo(
                            "Editor closed without saving.", err=True
                        )
                        continue
                    prompt = edited_prompt.strip()
                if prompt.strip().startswith("!fragment "):
                    prompt, fragments, attachments = (
                        process_fragments_in_chat(db, prompt)
                    )
                ...
                if template_obj:
                    try:
                        ...
                        template_prompt, template_system = (
                            template_obj.evaluate(input_, params)
                        )
    ...
    def inject(message, conversation_id, prompt_label, model_id, database):
        ...
        if conversation_id is None:
            ...
            try:
                conv_row = cast(
                    sqlite_utils.db.Table, db["conversations"]
                ).get(conversation_id)
        ...
        click.echo(
            "Injected message into conversation {}".format(conversation_id)
        )
